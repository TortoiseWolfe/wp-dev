...
const MetronomeApp = () => {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto bg-gray-100 rounded-lg shadow-md">
      <h1 className="text-2xl font-bold mb-4 text-center">4-Track Metronome</h1>
      
      {/* Preset Selector */}
      <div className="mb-4">
        <label className="block font-semibold mb-2">Beat Pattern:</label>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <label className="font-semibold">Tempo: {tempo} BPM</label>
          <button 
            onClick={togglePlay}
      ...
      <div className="space-y-4">
        {trackState.map((track, trackIndex) => (
          <div key={trackIndex} className="bg-white p-3 rounded shadow">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center">
                <button 
                  onClick={() => toggleMute(trackIndex)}
                  className={`p-1 mr-2 rounded-full ${track.muted ? 'bg-gray-300' : 'bg-blue-500'} text-white`}
                >
            ...
            <div className="grid grid-cols-8 gap-2">
              {track.pattern.map((on, stepIndex) => (
                <div 
                  ...
                  className={`
                    h-8 rounded cursor-pointer border-2 
                    ${on ? 'bg-blue-500 border-blue-600' : 'bg-gray-200 border-gray-300'} 
                    ${activeStep === stepIndex && isPlaying ? 'ring-2 ring-yellow-400' : ''}
                  `}
                />
              ))}
            </div>
